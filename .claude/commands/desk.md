...
7. **DASHBOARD REFRESH (the only link between the desk and the page):** after §0/§6/§8 are updated, write this run's live data to `~/Documents/Coding/agent-trading-desk/dashboard/data.json` (gitignored — stays local). Include: account KPIs (equity, day %, cash, HWM); positions (symbol/type/qty/cost/last/pnl_pct/lane) from the SYNC pull; the §9f equity-vs-QQQ curve as `equity_curve` rows {date, account, benchmark}; the most recent §8 entries as `decisions` [{ts, text}] (the reasoning feed); the §7 watchlist [{symbol, note, catalyst_date}]. Match the schema in `dashboard/data.example.json`. The page polls it every 30 min — no restart needed. (If the desk hasn't traded/changed, still refresh so "updated" stays current.)
8. **RISK-GATE STATE (feeds the PreToolUse gate, once wired):** also write `~/Documents/Coding/agent-trading-desk/desk_state.json` (gitignored) with this run's `{equity, open_premium, day_pnl_pct, drawdown_from_hwm_pct, consecutive_losses, lotto_position_count, theme_position_count, pending_lane}`. The `hooks/pretrade_gate.py` PreToolUse hook reads this to enforce the caps + kill switch on EVERY order — without current state it can only do per-order checks. Keep it fresh each cycle.
